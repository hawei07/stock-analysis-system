...
function fillFeeConfigForm() {
  document.getElementById('commissionRateInput').value = (Number(PortfolioState.feeConfig.commission_rate || 0) * 100).toFixed(4);
  document.getElementById('minCommissionInput').value = Number(PortfolioState.feeConfig.min_commission || 0).toFixed(2);
  document.getElementById('stampTaxRateInput').value = (Number(PortfolioState.feeConfig.stamp_tax_rate || 0) * 100).toFixed(4);
  document.getElementById('transferFeeRateInput').value = (Number(PortfolioState.feeConfig.transfer_fee_rate || 0) * 100).toFixed(4);
}
...
async function openAllocationModal() {
  ...
  if (!PortfolioState.portfolioData.positions || !PortfolioState.portfolioData.positions.length) {
    await loadPortfolio();
  }
...
function renderAllocationPie() {
  ...
  if (!el) return;
  if (!PortfolioState.charts.allocation) PortfolioState.charts.allocation = echarts.init(el);
  const summary = PortfolioState.portfolioData.summary || {};
  const rows = (PortfolioState.portfolioData.positions || [])
  ...
  if (!rows.length) {
    PortfolioState.charts.allocation.setOption({
  ...
  PortfolioState.charts.allocation.resize();
}
...
function privateMoney(v) {
  return PortfolioState.amountPrivacyMode ? maskedMoney() : money(v);
}

function signedPrivateMoney(v) {
  if (v == null || Number.isNaN(Number(v))) return '--';
  if (PortfolioState.amountPrivacyMode) return maskedMoney();
  const n = Number(v);
  return (n > 0 ? '+' : '') + money(n);
}
...
function updateAmountPrivacyButton() {
  ...
  if (btn) {
    btn.innerHTML = PortfolioState.amountPrivacyMode
    ...
    btn.setAttribute('aria-label', PortfolioState.amountPrivacyMode ? '显示金额' : '隐藏金额');
    btn.title = PortfolioState.amountPrivacyMode ? '显示金额' : '隐藏金额';
    btn.classList.toggle('active', PortfolioState.amountPrivacyMode);
  }
}

function toggleAmountPrivacy() {
  PortfolioState.amountPrivacyMode = !PortfolioState.amountPrivacyMode;
...
function calculateTradeFeeEstimate(amount, type, isDomestic) {
  ...
  const commissionRaw = amount * Number(PortfolioState.feeConfig.commission_rate || 0);
  const minCommission = Number(PortfolioState.feeConfig.min_commission || 0);
  const commission = commissionRaw > 0 && commissionRaw < minCommission ? minCommission : commissionRaw;
  const stampTax = type === 'sell' ? amount * Number(PortfolioState.feeConfig.stamp_tax_rate || 0) : 0;
  const transferFee = amount * Number(PortfolioState.feeConfig.transfer_fee_rate || 0);
